	...
	"template_entries": [
		{
			...
			"view_entries": [
				...
				},
				{
					...
					"parent_queries": [
						"select casenumber, statusdate, nextstatusdate,  eventstatusmappingcodede, casecategorydescription as casecategorydescription_, casetypedescription as casetypedescription_, orgunitname as orgunitname_, judgeid as judgeid_, max(case(isopen='true' AND casecyclesequence=1 AND casecycle>1, statusdate)) over (partition by casenumber) as last_opened, max(case(isactive='true' AND isopen='true' AND casecyclesequence>1, statusdate)) over (partition by casenumber) as last_reactivated, max(case(isactive=false , statusdate)) over (partition by casenumber) as last_closed, isopen, isactive, casebacklog |> select casenumber, max(statusdate) as last_statusdate, max(casecategorydescription_) as casecategorydescription, max(casetypedescription_) as casetypedescription, max(orgunitname_) as orgunitname, max(judgeid_) as judgeid,  sum(case(isactive='false' AND isopen='true',date_diff_d(nextstatusdate, statusdate), true, 0)) as days_inactive, date_diff_d({END_DATE}, max(case(last_opened is not null, last_opened, true, last_reactivated))) as days_pending,  sum(casebacklog) as casebacklogsum, (days_pending-days_inactive) as days_active_pending group by casenumber having casebacklogsum  > 0"
					],
	...
	"view_entries": [
		{
			...
			"parent_queries": [
				"select count(hearingdate) as total_hearing_dates,casenumber,max(dispositiondate) as disposition_date,min(hearingdate) as first_hearing,max(hearingdate) as last_hearing, max(casetypedescription) as last_casetypedescription, max(casecategorydescription) as last_casecategorydescription, max(casecategorydescription) as last_casecategorymappingdescription, max(hearingjudgeid) as last_hearingjudgeid, max(orgunitname) as last_orgunitname group by casenumber |> select disposition_date,total_hearing_dates,casenumber,first_hearing,last_hearing, last_casetypedescription, last_casecategorydescription, last_casecategorymappingdescription, last_hearingjudgeid, last_orgunitname, case(total_hearing_dates < 3, 1, total_hearing_dates >= 3, 0) as certainty_count"
			],
			"aggregate_type": "",
			"precision": "1",
			"prefix": "",
